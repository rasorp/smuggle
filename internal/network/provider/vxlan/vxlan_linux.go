...
func (p *Provider) DeleteRemote(
	...
	// Remove the ip rules for each local subnet.
...
func (p *Provider) SetRemote(
	...
	// from that subnet into the smuggle routing table. This ensures replies from
	// local containers to a remote host's physical IP are routed via vxlan0.
	for _, localSubnet := range req.LocalSubnets {
		if localSubnet.IPv4Network == nil {
			continue
		}
		...
		p.logger.Debug("added policy routing rule",
			zap.String("src_subnet", localSubnet.IPv4Network.String()),
			zap.Int("table", smuggleRoutingTableID),
		)
	}
