...
func (p *Provider) createIPv4(
	...
	// Disable rp_filter on the physical host interface explicitly. The kernel computes the
	// effective rp_filter per interface as max(conf/all, conf/<iface>), so setting conf/all=0
	// alone is insufficient if the interface has its own non-zero value (inherited from
	// conf/default, which is typically 1 or 2 on EC2 and modern Linux distributions).
	// Without this, asymmetric routing causes the host to silently drop return packets whose
	// reverse path resolves to the VXLAN interface rather than the physical one.
	...
	// Set the global baseline to 0. This does not override per-interface values but ensures
	// any newly created interfaces inherit a permissive default.
